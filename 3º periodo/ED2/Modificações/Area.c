...
#include "Registro.h"
	...
	struct tipoarea
	{
		Registro * itens; 	//um vetor dinamico para registros
...
void InsereItem(Registro A, TipoArea *Area, int indice_posicao, int ordena_ativa, Analise* estatistica)
{
    ...
    { 
 	...
 	Area->itens[Area->NumCelOcupadas] = A; 
...
{ 
    ...
    for(int i = 0; i < Area->NumCelOcupadas-1; i++){
...
{ 
	...
	/*retornando valor a ser removido */
	*A = Area->itens[Area->NumCelOcupadas -1];
